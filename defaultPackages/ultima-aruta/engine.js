...
let TILE_W, TILE_H, CHUNK_SIZE, MOVE_MS, CREATURE_MOVE_MS, DAY_MS, ELEV_PX, PERSP_STRENGTH, DUNGEON_SIZE;
let BIOMES, DUNGEON_BIOMES, ALL_BIOMES, FEATURES, CREATURE_DEFS, CREATURES, DUNGEON_CREATURES;
let VILLAGE, ITEMS, ITEM_DROPS, DIALOGS;
...
export function setup(mod) {
    ...
    ITEM_DROPS = mod.ITEM_DROPS; DIALOGS = mod.DIALOGS;
}
...
export function fbm2D(x, y, seed, octaves = 4, persistence = 0.5) {
...
export function _nightFactor(t) {
...
export function _visionRadius(t) {
...
export function drawFogOfWar(ctx, W, H, playerScreenX, playerScreenY, timeOfDay) {
...
export function camera(canvasW, canvasH, pwx, pwy) {
    const p = iso(pwx, pwy);
...
export function perspScale(screenY, viewH) {
...
let _sfxCtx = null;
export function _sfx(freq, dur = 0.1, type = 'sine', gain = 0.06) {
...
export function sfxHit()     { _sfx(180, 0.1, 'square', 0.07); }
export function sfxKill()    { _sfx(440, 0.08); setTimeout(() => _sfx(660, 0.12), 60); }
export function sfxHurt()    { _sfx(120, 0.14, 'sawtooth', 0.08); }
export function sfxLevelUp() { _sfx(523, 0.12); setTimeout(() => _sfx(659, 0.14), 100); setTimeout(() => _sfx(784, 0.2), 240); }

export function drawTile(ctx, sx, sy, biome, elev = 0.5) {
...
export function drawTileDepth(ctx, sx, sy, elev, elevS, elevE) {
    const lift = (elev - 0.35) * ELEV_PX;
    const ty = sy - lift;
    const hx = TILE_W / 2, hy = TILE_H / 2;
...
export function _adjustBright(hex, factor) {
    const h = hex.replace('#', '');
...
export function drawShadow(ctx, sx, sy, spriteSize) {
...
export function _getEmojiImg(emoji, size) {
...
export function drawEmoji(ctx, sx, sy, emoji, size = 28) {
